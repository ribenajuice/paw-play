...
/**
 * One drawn thing on the dish. Its [shape] (200 x 160 grid) is its VISIBLE outline, and is also its tap area,
 * so "tap it on the dish to take it off" needs no separate hit logic. By default the shape is [bounds] (or the
 * ellipse inside them when [oval]); pieces whose outline is not a box (a bun's dome, a wavy lettuce leaf)
 * give their true outline so empty corners never count as a hit.
 */
class DishPiece(
    ...
    val shape: Silhouette = shape ?: if (oval) {
        Silhouette(Oval(bounds.center.x, bounds.center.y, bounds.width / 2f, bounds.height / 2f))
    } else {
        Silhouette(Box(bounds.left, bounds.top, bounds.width, bounds.height, 0f))
    }
}
...
/**
 * How far (dish grid units, about dp) outside a piece a tap still counts. Some slabs are only 5 to 7 units tall
 * on the dish, so a small miss near one should take that one off, not nothing.
 */
const val DISH_TAP_SLOP = 6f

/**
 * Which ingredient was tapped on the dish. Two rules, in order:
 * 1. The topmost piece whose visible outline contains the tap wins, so the bun's crown never steals a tap that
 *    landed on the slab under it or in the air beside its dome.
 * 2. Otherwise the piece whose outline is nearest, if it is within [DISH_TAP_SLOP] (topmost on a tie).
 * Plates and stands (no ingredient) never count.
 */
fun hitTestDish(pieces: List<DishPiece>, point: Offset): String? {
    val ingredients = pieces.filter { it.ingredient != null }
    ingredients.lastOrNull { it.shape.contains(point.x, point.y) }?.let { return it.ingredient }
    var best: DishPiece? = null
    var bestDistance = DISH_TAP_SLOP
    for (piece in ingredients) {
        val d = piece.shape.distance(point.x, point.y)
        if (d <= bestDistance) {
            best = piece
            bestDistance = d
        }
...
private fun burger(): DishSpec {
    ...
    val view = DishView { on ->
        ...
        if (BURGER_BUN in on) { // the heel; the crown is added last
            val y = top - 18f
            out += DishPiece(BURGER_BUN, Rect(40f, y, 160f, top), shape = Silhouette(Box(40f, y, 120f, 18f, 9f))) {
                rrect(40f, y, 120f, 18f, 9f, BunHeel, rimWidth = 3f)
            }
            top -= 16f
        }
        if (BURGER_PATTY in on) {
            val y = top - 20f
            out += DishPiece(BURGER_PATTY, Rect(34f, y, 166f, top), shape = Silhouette(Box(34f, y, 132f, 20f, 10f))) {
        ...
        if (BURGER_CHEESE in on) {
            val t = top
            val cheesePath = "M34,${t - 7}H166V${t - 1}L148,${t - 1}L142,${t + 9}L136,${t - 1}L66,${t - 1}L60,${t + 9}L54,${t - 1}L34,${t - 1}Z"
            out += DishPiece(BURGER_CHEESE, Rect(34f, t - 7f, 166f, t + 9f), shape = Silhouette(polyFromPath(cheesePath))) {
                shape(cheesePath, CheeseBurger)
            }
            top -= 6f
        }
        if (BURGER_TOMATO in on) {
            val t = top
            out += DishPiece(BURGER_TOMATO, Rect(42f, t - 11f, 158f, t), shape = Silhouette(Box(42f, t - 11f, 116f, 11f, 5.5f))) {
        ...
        if (BURGER_LETTUCE in on) {
            val t = top
            val y = t - 6f
            val lettucePath = "M30,$y Q40,${y - 10} 50,$y T70,$y T90,$y T110,$y T130,$y T150,$y T170,$y V${y + 9} H30 Z"
            out += DishPiece(BURGER_LETTUCE, Rect(30f, y - 10f, 170f, y + 9f), shape = Silhouette(polyFromPath(lettucePath))) {
                shape(lettucePath, LettuceGreen)
            }
            top -= 10f
        }
        if (BURGER_PICKLE in on) {
            val t = top
            out += DishPiece(
                BURGER_PICKLE, Rect(44f, t - 10f, 156f, t + 2f),
                shape = Silhouette(listOf(62f, 100f, 138f).map { Oval(it, t - 4f, 18f, 5.5f) }),
            ) {
        ...
        if (BURGER_BUN in on) { // crown, always on top: a bun with toppings is a closed burger
            val t = top
            // Only the dome itself counts as the crown, not the empty corners of its bounding box.
            val domePath = "M40,${t + 2}C40,${t - 36} 66,${t - 40} 100,${t - 40}C134,${t - 40} 160,${t - 36} 160,${t + 2}Z"
            out += DishPiece(BURGER_BUN, Rect(40f, t - 40f, 160f, t + 2f), shape = Silhouette(polyFromPath(domePath))) {
                shape(domePath, BunTop)
                sesame(
                    listOf(
...
private fun iceCream(): DishSpec {
    ...
    val view = DishView { on ->
        ...
        for (id in listOf(ICE_STRAWBERRY, ICE_CHOCOLATE)) {
            ...
            crest = y + 6f
            val icon = icons.getValue(id)
            val outline = Silhouette(Placed(shapes.getValue(id), 62f, y, 76f, 60f))
            scoops += DishPiece(id, Rect(62f, y, 138f, y + 60f), shape = outline) { placeIcon(icon, 62f, y, 76f, 60f) }
            top = top - 42f + 12f // scoops nest 12 into each other
        }
        if (ICE_WAFER in on) { // leans out behind the scoops
            val ty = crest ?: (top - 20f)
            out += DishPiece(
                ICE_WAFER, Rect(108f, ty - 26f, 170f, ty + 36f),
                shape = Silhouette(Placed(WaferShape, 108f, ty - 26f, 62f)),
            ) { placeIcon(WaferIcon, 108f, ty - 26f, 62f) }
        }
        if (ICE_CONE in on) {
            val y0 = base - 54f
            val conePath = "M72,${y0}L128,${y0}L104,${base - 4}Q100,${base + 2} 96,${base - 4}Z"
            out += DishPiece(
                ICE_CONE, Rect(68f, y0 - 4f, 132f, base + 2f),
                shape = Silhouette(polyFromPath(conePath), Box(68f, y0 - 4f, 64f, 11f, 5.5f)),
            ) {
        ...
        if (ICE_CHERRY in on) {
            val ty = crest ?: top
            out += DishPiece(
                ICE_CHERRY, Rect(83f, (ty - 27f).coerceAtLeast(0f), 117f, ty + 7f),
                shape = Silhouette(Placed(CherryShape, 83f, ty - 27f, 34f)),
            ) { placeIcon(CherryIcon, 83f, ty - 27f, 34f) }
        }
        if (ICE_SPRINKLES in on) {
            val ty = crest ?: (top - 4f)
            // The sprinkles are a loose scatter, so the whole patch they cover is the tap area (a generous oval).
            out += DishPiece(ICE_SPRINKLES, Rect(78f, ty + 2f, 126f, ty + 28f), oval = true) {
