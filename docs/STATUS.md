...
- **Latest build**: `main` at 17b829b passed CI (real Gradle lint, unit tests and debug build) and a manually run `Release build` (run 36103617834, 2026-09-25, about 8.4 MB download named `release-build`). It is an unsigned test build (no signing keystore yet) that includes all six games. Not yet confirmed installed on the founder's phone.
- **Currently in flight**: nothing, and no open PRs. The merged feature branches (`feat/paw-pour`, `feat/paw-kitchen`, `feat/paw-trace`, `feat/paw-blocks`, `feat/paw-pop`) and `docs/status-all-games-merged` still exist on GitHub and can be deleted. Four leftover local working folders sit in `.claude/worktrees/` (untracked, safe to clear).
- **Blocked on founder**:
