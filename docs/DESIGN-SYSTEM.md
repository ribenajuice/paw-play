...
| Sparkle wave | instant, about 1.5 s | A wavy band across the full width, centre line `y0 + 11 sin(2 pi x / 96 + 3 t)` dp, drawn as three round polylines (46dp white 16%, 28dp sunshine 30%, 10dp white 80%) with ten sparkles (radius 5 to 9) riding on it; it rises from just below the ship's nose to above the top edge in about 1.5 s (about 490 dp/s) and pops each ordinary target as its centre is reached (only if 3 or more were on screen when the gift arrived; if fewer, the band still glitters and pops nothing). Carriers are skipped. No glow. |
